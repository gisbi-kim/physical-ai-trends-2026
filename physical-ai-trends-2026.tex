...
\newcommand{\watch}[2]{%
  \noindent\begin{tabularx}{\textwidth}{>{\sffamily\bfseries\color{teal}}p{0.28\textwidth}X}
  #1 & #2
  \end{tabularx}\par
}
...
\begin{tabularx}{\textwidth}{>{\sffamily\bfseries\color{teal}}p{0.28\textwidth}X}
정책 & 새 과업 조합, 기체 전이, action latency, 비동기 실행 \\
세계 상태 & 3D·occupancy·촉각의 planner 기여와 불확실성 calibration \\
장기성 & memory retrieval, checkpoint, partial completion, recovery \\
월드모델 & counterfactual planning과 실제 폐루프 위험 감소 \\
데이터 & 인간·로봇·합성 데이터의 정렬과 재검증 \\
신뢰성 & intervention, abstention, near-miss, runtime verification \\
운영 & edge 실행, uptime, change failure, canary, rollback, 현장 총비용 \\
\end{tabularx}
\end{fieldbox}
